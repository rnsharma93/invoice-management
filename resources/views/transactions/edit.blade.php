...
@section('main-content')
    ...
    <div class="container-fluid">
        <h1 class="h3 mb-4 text-gray-800">Edit Transaction ({{ ucfirst($transaction->type) }})</h1>
                ...
                <form action="{{ route('transactions.update', $transaction->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="out_type" id="type_vendor" value="vendor" {{ $transaction->vendor_id ? 'checked' : '' }}>
                                    <label class="form-check-label" for="type_vendor">Vendor</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="out_type" id="type_vehicle" value="vehicle" {{ $transaction->vehicle_id ? 'checked' : '' }}>
                        ...
                        <div class="form-group" id="vendor_group" style="{{ $transaction->vehicle_id ? 'display: none;' : 'display: block;' }}">
                            ...
                            <select name="vendor_id" id="vendor_id" class="form-control">
                                ...
                                @foreach($vendors as $vendor)
                                    <option value="{{ $vendor->id }}" {{ $transaction->vendor_id == $vendor->id ? 'selected' : '' }}>{{ $vendor->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group" id="vehicle_group" style="{{ $transaction->vendor_id ? 'display: none;' : 'display: block;' }}">
                            ...
                            <select name="vehicle_id" id="vehicle_id" class="form-control">
                                ...
                                @foreach($vehicles as $vehicle)
                                    <option value="{{ $vehicle->id }}" {{ $transaction->vehicle_id == $vehicle->id ? 'selected' : '' }}>{{ $vehicle->vehicle_number }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="method" id="method" class="form-control" required>
                            ...
                            <option value="bank_transfer" {{ $transaction->method == 'bank_transfer' ? 'selected' : '' }}>Bank Transfer</option>
                            <option value="cheque" {{ $transaction->method == 'cheque' ? 'selected' : '' }}>Cheque</option>
                            <option value="credit_card" {{ $transaction->method == 'credit_card' ? 'selected' : '' }}>Credit Card</option>
                            <option value="other" {{ $transaction->method == 'other' ? 'selected' : '' }}>Other</option>
                        </select>
                    </div>
